...
module MEM(
    ...
    // for trace test
    input   wire[31:0]  mem_output_data_i,  // 读入外部dmem的数据，替换原主存输出
    output  wire[31:0]  mem_input_data_o    // 输出至外部dmem的数据
    );
    
//    wire [31:0] mem_addr_tmp = mem_addr_i - 16'h4000;
    wire [31:0] mem_addr_tmp = mem_addr_i;

    reg  [31:0] dram_input;
    wire [31:0] dram_output;
    
    // 将外部送入的数据替换dram输出
    assign dram_output = mem_output_data_i;
    ...
    always @(*) begin
        case(mem_data_sel_i)
            3'b000: mem_data_o = {{24{dram_output_byte[7]}}, dram_output_byte};     // lb
            3'b001: mem_data_o = {{16{dram_output_word[15]}}, dram_output_word};    // lh
            3'b010: mem_data_o = dram_output_dword;                                 // lw
            3'b100: mem_data_o = {24'b0,dram_output_byte};                          // lbu
            3'b101: mem_data_o = {16'b0,dram_output_word};                          // lhu
            default:;
        endcase
    end
...
//    );
    ...
    // for trace test
    assign mem_input_data_o = dram_input; 
